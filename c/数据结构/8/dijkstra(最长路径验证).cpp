...
{	int dist[MAXV],path[MAXV];
	memset(dist,0,sizeof(dist));	//dist所有元素初始化为0 
	...
	for (i=0;i<g.n-1;i++)		//循环直到所有顶点的最长路径都求出
	{	maxdis=0;				//maxdis置最大长度初值
		for (j=0;j<g.n;j++)		//选取U中且具有最大最长路径长度的顶点u
			if (S[j]==0 && dist[j]<INF && dist[j]>maxdis) 
			{	u=j;
				maxdis=dist[j];
			}
		printf("  找到最长距离的顶点u=%d\n",u); 
		S[u]=1;					//顶点u加入S中
		for (j=0;j<g.n;j++)		//修改不在S中（即U中）的顶点的最长路径
	...
	Dispath(g,dist,path,S,v);	//输出最长路径
}
